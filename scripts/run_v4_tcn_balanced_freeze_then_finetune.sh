...
LOG_DIR="${LOG_DIR:-reports/logs/train_rul_hybrid_v4_tcn_balanced}"
mkdir -p "$LOG_DIR"

RUN_ID="$(date +%Y%m%d_%H%M%S)_v4tcn_balanced_freeze_then_finetune"
TRAIN_SCRIPT="src/prediction/train_rul_hybrid_v4_tcn.py"

# Defaults target the new V4_TCN heads on the heavier 2048 window.
# Override examples:
#   WINDOW_SIZES="1024 2048" ./scripts/run_v4_tcn_balanced_freeze_then_finetune.sh
#   TEMPORAL_TYPES="lstm gru transformer tcn" ./scripts/run_v4_tcn_balanced_freeze_then_finetune.sh
WINDOW_SIZES_RAW="${WINDOW_SIZES:-2048}"
TEMPORAL_TYPES_RAW="${TEMPORAL_TYPES:-tcn tcn_ms tcna tcn_bi}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-XJTU_SY_RUL_HybridV4_TCN_Balanced}"
...
echo "${GREEN}[INFO] All requested train_rul_hybrid_v4_tcn balanced runs finished.${RESET}"
