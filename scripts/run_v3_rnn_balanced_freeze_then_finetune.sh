...
LOG_DIR="${LOG_DIR:-reports/logs/train_rul_hybrid_v3_rnn_balanced}"
mkdir -p "$LOG_DIR"

RUN_ID="$(date +%Y%m%d_%H%M%S)_v3rnn_balanced_freeze_then_finetune"
TRAIN_SCRIPT="src/prediction/train_rul_hybrid_v3_rnn.py"

# Defaults mirror the old ws2048 helper, but target the new V3_RNN heads.
# Override examples:
#   WINDOW_SIZES="1024 2048" ./scripts/run_v3_rnn_balanced_freeze_then_finetune.sh
#   TEMPORAL_TYPES="lstm gru transformer bilstm" ./scripts/run_v3_rnn_balanced_freeze_then_finetune.sh
WINDOW_SIZES_RAW="${WINDOW_SIZES:-1024}"
TEMPORAL_TYPES_RAW="${TEMPORAL_TYPES:-lstm bilstm lstm_attn bigru gru_attn transformer_improved}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-XJTU_SY_RUL_HybridV3_RNN_Balanced}"
...
echo "${GREEN}[INFO] All requested train_rul_hybrid_v3_rnn balanced runs finished.${RESET}"
