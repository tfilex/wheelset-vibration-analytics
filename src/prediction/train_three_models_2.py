"""
train_three_models_2.py — расширенный pipeline для трех temporal-моделей RUL.
...
Запуск (полный):
    uv run python src/prediction/train_three_models_2.py --profile full

Запуск (быстрый sanity-check перед ночным прогоном):
    uv run python src/prediction/train_three_models_2.py --profile fast

Запуск (рекомендуемый дневной режим):
    uv run python src/prediction/train_three_models_2.py --profile balanced

Запуск с именем MLflow-эксперимента:
    uv run python src/prediction/train_three_models_2.py --profile balanced --experiment-name "my_experiment"
...
MLFLOW_EXPERIMENT = "XJTU_SY_RUL_ThreeModels_v2_plots_examlpes_frozen"
...
FIGURES_BASE_DIR = "reports/figures/summary/train_three_models_2_frozen"
...
def run_for_window_size(
    ...
    *,
    ...
    with mlflow.start_run(run_name=f"three_models_v2_{run_tag}_ws{cfg.window_size}"):
        mlflow.log_params(
            {
        ...
        for temporal_type in temporal_types:
            ...
            study = optuna.create_study(
                direction="minimize",
                study_name=f"three_models_v2_{run_tag}_{
...
def main() -> None:
    ...
    print("\n" + "=" * 78)
    print(f"  TRAINING COMPLETE (THREE_MODELS_V2) — {mode_label}")
    print("=" * 78)
    for ws in window_sizes:
        print(f"\n[window_size={ws}]")
        for model_name in temporal_types:
