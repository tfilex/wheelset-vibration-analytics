"""
train_rul_hybrid_v2.py — CNN+temporal pipeline для прогнозирования RUL.
...
Запуск (полный):
    uv run python src/prediction/train_rul_hybrid_v2.py --profile full

Запуск (быстрый sanity-check перед ночным прогоном):
    uv run python src/prediction/train_rul_hybrid_v2.py --profile fast

Запуск (рекомендуемый дневной режим):
    uv run python src/prediction/train_rul_hybrid_v2.py --profile balanced

Запуск с именем MLflow-эксперимента:
    uv run python src/prediction/train_rul_hybrid_v2.py --profile balanced --experiment-name "my_experiment"
...
Режим --profile fast:
    ...
    - по умолчанию используется кэш замороженных CNN-фичей;
      для старого пути через скалограммы используй --no-feature-cache
"""
...
MLFLOW_EXPERIMENT = "XJTU_SY_RUL_HybridTemporal_v2_frozen"
...
PREDS2_MODELS_DIR = os.path.join(MODELS_DIR, "preds_2_frozen")
...
FIGURES_BASE_DIR = "reports/figures/summary/train_rul_hybrid_v2_frozen"
...
def evaluate_with_predictions(
    ...
    if (
        ...
        return float("inf"), float("inf"), preds, labels_list
    mae = mean_absolute_error(labels_list, preds)
    return mse, mae, preds, labels_list
...
def objective(
    ...
    *,
    ...
    if use_feature_cache:
        ...
        model = build_model(
            temporal_type, hidden_size, dropout, device, fine_tune=True)
            ...
            mlflow.log_params(
                {
                    ...
                    "fine_tune_cnn": not use_feature_cache,
                    "cnn_checkpoint_path": CNN_CHECKPOINT_PATH,
                    "encoder_dim": encoder_dim or "image_input",
                }
            )
    ...
    *,
    ...
    print(
        color_text(
            ...
            f"batch={batch_size}, hidden={int(params['hidden_size'])}, "
            f"dropout={float(params['dropout']):.4f}, lr={float(params['lr']):.6g}, "
            f"feature_cache={int(use_feature_cache)}, fine_tune_cnn={int(not use_feature_cache)}",
            "cyan",
            bold=True,
        )
    )
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        print(
            color_text(
                ...
                f"best_val_mse={best_val_mse:.6f}, epochs_ran={len(train_hist)}",
                "green",
                bold=True,
            )
        )

        test_mse, test_mae, test_preds, test_labels = evaluate_with_predictions(
...
def run_for_window_size(
    ...
    *,
    ...
    hpo_val_ds: Dataset = val_ds
    hpo_test_ds: Dataset = test_ds
    if use_feature_cache:
        encoder_dim = precompute_cnn_feature_cache(
            train_ds, val_ds, test_ds, device)
        hpo_train_ds = FeatureBearingRULDataset(
            train_dirs,
            seq_length=cfg.seq_length,
            window_size=cfg.window_size,
            seq_stride=cfg.seq_stride,
        ...
        hpo_val_ds = FeatureBearingRULDataset(
            val_dirs,
            seq_length=cfg.seq_length,
            window_size=cfg.window_size,
            seq_stride=cfg.val_test_stride,
        ...
        hpo_test_ds = FeatureBearingRULDataset(
            test_dirs,
            seq_length=cfg.seq_length,
            window_size=cfg.window_size,
            seq_stride=cfg.val_test_stride,
    ...
    with mlflow.start_run(
        run_name=f"rul_hybrid_v2_{run_tag}_{feature_status}_ws{cfg.window_size}"
    ):
        mlflow.log_params(
            {
    ...
    for temporal_type in temporal_types:
        print("\n" + "=" * 70)
        print(color_text(
            f"[INFO] Final fit for {temporal_type.upper()} with best params",
            "cyan",
            bold=True,
        ))
        print("=" * 70)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Use precomputed frozen CNN features. Disable with --no-feature-cache.",
    )
    ...
    if profile == "fast":
        ...
        print(f"  trials={n_trials}, epochs={epochs}, "
              f"cwt_scales={cwt_scales}, stride={seq_stride}, workers={NUM_WORKERS}")
        print("!" * 78)
    ...
    print("\n" + "=" * 78)
    print(color_text(
        f"  TRAINING COMPLETE (RUL_HYBRID_V2) — {mode_label}",
        "green",
        bold=True,
    ))
    print("=" * 78)
