"""
train_rul_hybrid_v5_odd.py — CNN+temporal pipeline для прогнозирования RUL.
...
2) Optuna HPO:
   - подбор гиперпараметров отдельно для каждой temporal-архитектуры:
     lstm, gru, transformer и SOTA-голов.
...
Архитектуры:
   ...
   - patchtst    — PatchTST: патч-токенизация + Transformer (Nie et al., ICLR 2023)
   - conformer   — Conformer: CNN + MHSA гибрид (Gulati et al., 2020)
   - mamba       — Mamba SSM: selective state space model (Gu & Dao, 2024),
                   требует mamba-ssm, при отсутствии пропускается

Запуск (полный):
    uv run python src/prediction/train_rul_hybrid_v5_odd.py --profile full

Запуск (быстрый sanity-check перед ночным прогоном):
    uv run python src/prediction/train_rul_hybrid_v5_odd.py --profile fast

Запуск (рекомендуемый дневной режим):
    uv run python src/prediction/train_rul_hybrid_v5_odd.py --profile balanced

Запуск с именем MLflow-эксперимента:
    uv run python src/prediction/train_rul_hybrid_v5_odd.py --profile balanced --experiment-name "my_experiment"
...
10) Discriminative learning rate: CNN encoder обучается с меньшим LR.
...
try:
    from mamba_ssm import Mamba as _MambaLayer
    MAMBA_AVAILABLE = True
except ImportError:
    MAMBA_AVAILABLE = False
    print("[WARN] mamba-ssm не установлен. "
          "Архитектура 'mamba' будет пропущена. "
          "Установить: pip install mamba-ssm")
...
class PatchTSTHead(nn.Module):
    ...
    Разбивает временной ряд на патчи → линейная проекция → Transformer.
    Хорошо улавливает локальные паттерны деградации.
    ...
    def __init__(self, input_size: int, hidden_size: int,
                 ...
                 patch_size: int = 8, nhead: int = 4):
        super().__init__()
        self.patch_size = patch_size
        # Линейная проекция одного патча в d_model
        self.patch_embed = nn.Linear(patch_size * input_size, hidden_size)
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True,
            norm_first=True,   # Pre-LN стабильнее для малых датасетов
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, T, C = x.shape
        # Pad до кратного patch_size
        pad = (self.patch_size - T % self.patch_size) % self.patch_size
        if pad > 0:
            x = F.pad(x, (0, 0, 0, pad))
        # Reshape: (B, num_patches, patch_size * C)
        x = x.reshape(B, -1, self.patch_size * C)
        x = self.patch_embed(x)        # (B, num_patches, hidden_size)
        x = self.transformer(x)        # (B, num_patches, hidden_size)
        x = x.mean(dim=1)              # mean-pool по патчам
        return self.fc(self.dropout(x))


class _ConformerBlock(nn.Module):
    """
    Один блок Conformer: Feed-Forward → MHSA → Depthwise Conv → Feed-Forward.
    Схема из Gulati et al. 2020 (ASR), адаптирована для RUL.
    """

    def __init__(self, d_model: int, nhead: int,
                 dropout: float, conv_kernel: int = 31):
        super().__init__()
        ...
        self.pw_expand = nn.Linear(d_model, d_model * 2)   # pointwise expand + GLU
        pad = (conv_kernel - 1) // 2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Conv module
        xn = self.norm_conv(x)
        xn = self.pw_expand(xn)                # (B, T, 2*D)
        xn = F.glu(xn, dim=-1)                 # (B, T, D) — GLU gate
        xn = self.dw_conv(xn.transpose(1, 2))  # (B, D, T)
        xn = F.silu(self.bn(xn)).transpose(1, 2)  # (B, T, D)
        xn = self.pw_project(xn)
        x = x + self.drop_conv(xn)
        # Feed-Forward 2
        x = x + 0.5 * self.ff2(x)
...
class ConformerHead(nn.Module):
    """
    Conformer для RUL prediction.
    Сочетает свёрточные локальные паттерны и глобальное внимание —
    доминирующий подход в задачах прогнозирования на вибросигналах.
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """
    ...
    def __init__(self, input_size: int, hidden_size: int,
        ...
        self.blocks = nn.ModuleList([
            _ConformerBlock(hidden_size, nhead, dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for block in self.blocks:
            x = block(x)
        return self.fc(x[:, -1])     # последний шаг → (B, 1)


class MambaHead(nn.Module):
    """
    Mamba SSM (Gu & Dao, 2024).
    Линейная сложность по времени, отлично работает на длинных
    последовательностях деградации.
    Требует: pip install mamba-ssm  (CUDA, Linux)
    При отсутствии пакета — выбрасывает ImportError при создании.
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """
    ...
    def __init__(self, input_size: int, hidden_size: int,
        ...
        if not MAMBA_AVAILABLE:
            raise ImportError(
                ...
                "Установите: pip install mamba-ssm  (требует CUDA + Linux)"
            )
        self.input_proj = nn.Linear(input_size, hidden_size)
        self.layers = nn.ModuleList([
            _MambaLayer(d_model=hidden_size, d_state=d_state,
                        d_conv=4, expand=2)
            for _ in range(num_layers)
        ])
...
_NEW_SOTA_TYPES = {"patchtst", "conformer", "mamba"}


class _LocalHybridRULNet(nn.Module):
    """CNN-энкодер + любая SOTA-голова для финального fine-tune."""
...
MLFLOW_EXPERIMENT = "XJTU_SY_RUL_HybridV5_SOTA"
...
PREDS_MODELS_DIR = os.path.join(MODELS_DIR, "preds_5_odd")
...
FIGURES_BASE_DIR = "reports/figures/summary/train_rul_hybrid_v5_odd"
...
def build_model(
    ...
    if temporal_type in _NEW_SOTA_TYPES:
        head = build_temporal_model(
            temporal_type, hidden_size, dropout,
            enc_dim, torch.device("cpu"), num_layers, params,
        )
...
def build_temporal_model(
    ...
    if temporal_type == "patchtst":
        patch_size = (params or {}).get("patch_size", 8)
        nhead = (params or {}).get("nhead", 4)
        return PatchTSTHead(
            input_size=encoder_dim, hidden_size=hidden_size,
            num_layers=num_layers, dropout=dropout,
            patch_size=patch_size, nhead=nhead,
        ).to(device)

    elif temporal_type == "conformer":
        nhead = (params or {}).get("nhead", 4)
        return ConformerHead(
            input_size=encoder_dim, hidden_size=hidden_size,
            num_layers=num_layers, dropout=dropout, nhead=nhead,
        ).to(device)

    elif temporal_type == "mamba":
        d_state = (params or {}).get("d_state", 16)
        return MambaHead(
            input_size=encoder_dim, hidden_size=hidden_size,
            num_layers=num_layers, dropout=dropout, d_state=d_state,
        ).to(device)
        ...
        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            ...
            if mono_weight > 0.0:
                loss = loss + mono_weight * \
                    monotonicity_penalty(outputs.reshape(-1))
...
def set_encoder_trainable(model: nn.Module, trainable: bool, layers: Optional[List[str]] = None) -> None:
    """Enables/disables gradients for the CNN encoder.

    If layers is provided, only encoder parameters containing those substrings
    are unfrozen.
    """
    ...
    for name, param in model.named_parameters():
        if "encoder" in name:
            if trainable and layers:
                param.requires_grad = any(layer in name for layer in layers)
            else:
...
def build_finetune_optimizer(
    ...
    *,
    base_lr: float,
    weight_decay: float,
    encoder_lr_mult: float = FINETUNE_ENCODER_LR_MULT,
) -> optim.Optimizer:
    """
    AdamW with discriminative LR:
    - CNN encoder: base_lr * encoder_lr_mult
    - temporal head/regression layers: base_lr
    - 1D params (biases, BatchNorm/LayerNorm) have weight_decay=0.0
    """
    encoder_decay = []
    encoder_no_decay = []
    head_decay = []
...
def objective(
    ...
    *,
    ...
    # Архитектурно-специфичные параметры
    if temporal_type in ("patchtst", "conformer"):
    ...
    model = build_temporal_model(
        ...
        num_layers=num_layers, params=sota_params)
    ...
    with mlflow.start_run(run_name=f"{temporal_type}_trial_{trial.number:03d}", nested=True):
        try:
            ...
            mlflow.log_params(
                {"nhead": nhead, "patch_size": patch_size, "d_state": d_state}
            )
    ...
    *,
    ...
    if freeze_cnn_final:
        ...
        optimizer = build_finetune_optimizer(
            ...
            base_lr=base_lr,
            weight_decay=weight_decay,
            encoder_lr_mult=FINETUNE_ENCODER_LR_MULT,
        )
    else:
        # Финальный fine-tune идёт end-to-end: CWT -> CNN -> temporal -> RUL.
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.log_params(
            {
                ...
                "use_feature_cache": freeze_cnn_final,
                "final_use_feature_cache": freeze_cnn_final,
                "requested_use_feature_cache": use_feature_cache,
                "fine_tune_cnn": not freeze_cnn_final,
                "finetune_warmup_epochs": FINETUNE_WARMUP_EPOCHS,
                "finetune_max_batch_size": FINETUNE_MAX_BATCH_SIZE,
                "encoder_lr_multiplier": FINETUNE_ENCODER_LR_MULT,
                "cnn_checkpoint_path": CNN_CHECKPOINT_PATH,
        ...
        for epoch in range(epochs):
            if (
                ...
                print(
                    color_text(
                        ...
                        f"encoder_lr={base_lr * FINETUNE_ENCODER_LR_MULT:.6g}, "
                        f"head_lr={base_lr:.6g}",
                        "green",
                        bold=True,
                    )
                )
            ...
            encoder_lr = lr_by_group.get(
                "encoder_decay",
                lr_by_group.get("encoder_no_decay", 0.0),
            )
            head_lr = lr_by_group.get(
                "head_decay",
                lr_by_group.get(
                    "head_no_decay",
                    lr_by_group.get("head", lr_by_group.get("group_0", 0.0)),
                ),
            )
            mlflow.log_metrics(
                {
            ...
            print(
                color_text(
                    ...
                    f"head_lr={head_lr:.3g} "
                    f"encoder_lr={encoder_lr:.3g} "
        ...
        ckpt_path = os.path.join(
            PREDS_MODELS_DIR,
            f"best_rul_{temporal_type}_ws{window_size}_v5odd{artifact_suffix}.pth",
        )
        torch.save(
            {
                "temporal_type": temporal_type,
...
def run_for_window_size(
    ...
    *,
    ...
    with mlflow.start_run(
        run_name=f"rul_hybrid_v5_odd_{run_tag}_{cli_feature_status}_ws{cfg.window_size}"
    ):
        mlflow.log_params(
            {
        ...
        for temporal_type in temporal_types:
            ...
            study = optuna.create_study(
                direction="minimize",
                study_name=(
                    f"rul_hybrid_v5_odd_{run_tag}_{cli_feature_status}_"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Hybrid V5 SOTA temporal RUL models.")
    parser.add_argument(
        "--fast",
        action="store_true",
        help=(
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Backward-compatible flag. In v5 HPO always uses frozen CNN "
    ...
    if "mamba" in temporal_types and not MAMBA_AVAILABLE:
        print("[WARN] 'mamba' пропущен — mamba-ssm не установлен.")
        temporal_types = [t for t in temporal_types if t != "mamba"]
    if not temporal_types:
        print("[WARN] Нет доступных temporal types для запуска.")
        return
    ...
    print("\n" + "=" * 78)
    print(color_text(
        f"  TRAINING COMPLETE (HYBRID_V5_ODD) — {mode_label}",
        "green",
        bold=True,
    ))
    print("=" * 78)
