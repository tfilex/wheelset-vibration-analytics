"""
train_rul_hybrid_v4_tcn.py — CNN+temporal pipeline для прогнозирования RUL.
...
2) Optuna HPO:
   - подбор гиперпараметров отдельно для каждой temporal-архитектуры:
     lstm, gru, transformer и TCN-вариантов.
...
Архитектуры:
   ...
   - tcn         — классический causal TCN (Bai et al. 2018)
   - tcn_ms      — multi-scale TCN (параллельные ветки ks=3,7,15)
   - tcna        — TCN + temporal attention
   - tcn_bi      — bidirectional TCN (оффлайн, без causal)

Запуск (полный):
    uv run python src/prediction/train_rul_hybrid_v4_tcn.py --profile full

Запуск (быстрый sanity-check перед ночным прогоном):
    uv run python src/prediction/train_rul_hybrid_v4_tcn.py --profile fast

Запуск (рекомендуемый дневной режим):
    uv run python src/prediction/train_rul_hybrid_v4_tcn.py --profile balanced

Запуск с именем MLflow-эксперимента:
    uv run python src/prediction/train_rul_hybrid_v4_tcn.py --profile balanced --experiment-name "my_experiment"
...
class TCNBlock(nn.Module):
    """Один causal residual-блок с дилатацией и WeightNorm."""
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        super().__init__()
        self.causal = causal
        padding = (kernel_size - 1) * dilation if causal else \
...
class TCNHead(nn.Module):
    """
    Классический TCN (Bai et al. 2018).
    Дилатации: 1, 2, 4, ..., 2^(num_layers-1).
    Causal — не использует будущие шаги.
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """

    def __init__(self, input_size: int, hidden_size: int,
                 num_layers: int, dropout: float, kernel_size: int = 3):
        super().__init__()
        layers, in_ch = [], input_size
        for i in range(num_layers):
            layers.append(TCNBlock(in_ch, hidden_size,
                                   kernel_size, 2 ** i, dropout, causal=True))
            in_ch = hidden_size
...
class MultiScaleTCNHead(nn.Module):
    """
    Multi-scale TCN: три параллельные ветки с kernel_size 3, 7, 15.
    Хорошо улавливает деградацию на разных временных масштабах.
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x.transpose(1, 2)
        outs = [b(x)[:, :, -1] for b in self.branches]
...
class TCNAttentionHead(nn.Module):
    ...
    После стека TCN-блоков — механизм внимания по временной оси.
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """
...
class BiTCNHead(nn.Module):
    """
    Bidirectional TCN — без causal padding, видит весь контекст.
    Лучшее качество на оффлайн-датасете (XJTU-SY).
    Вход:  (batch, seq_len, input_size)
    Выход: (batch, 1)
    """
    ...
    def __init__(self, input_size: int, hidden_size: int,
        ...
        for i in range(num_layers):
            layers.append(TCNBlock(in_ch, hidden_size,
                                   kernel_size, 2 ** i, dropout, causal=False))
            in_ch = hidden_size
        self.network = nn.Sequential(*layers)
        self.fc = nn.Linear(hidden_size, 1)
...
_NEW_TCN_TYPES = {"tcn", "tcn_ms", "tcna", "tcn_bi"}


class _LocalHybridRULNet(nn.Module):
    """CNN-энкодер + любая TCN-голова для финального fine-tune."""
...
MLFLOW_EXPERIMENT = "XJTU_SY_RUL_HybridV4_TCN"
...
PREDS_MODELS_DIR = os.path.join(MODELS_DIR, "preds_4_tcn")
...
FIGURES_BASE_DIR = "reports/figures/summary/train_rul_hybrid_v4_tcn"
...
def build_model(
    ...
    if temporal_type in _NEW_TCN_TYPES:
...
def build_temporal_model(
    ...
    if temporal_type == "tcn":
        return TCNHead(
            ...
            kernel_size=params.get("kernel_size", 3) if isinstance(params, dict) else 3,
        ).to(device)

    elif temporal_type == "tcn_ms":
        return MultiScaleTCNHead(
            input_size=encoder_dim, hidden_size=hidden_size,
            num_layers=num_layers, dropout=dropout,
        ).to(device)

    elif temporal_type == "tcna":
...
def objective(
    ...
    *,
    ...
    if temporal_type in ("tcn", "tcna", "tcn_bi"):
        kernel_size = trial.suggest_categorical("kernel_size", [3, 5, 7])
    else:
        kernel_size = 3
    params_dict = {"kernel_size": kernel_size}
    ...
    model = build_temporal_model(
        ...
        num_layers=num_layers, params=params_dict)
    ...
    *,
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        ckpt_path = os.path.join(
            PREDS_MODELS_DIR,
            f"best_rul_{temporal_type}_ws{window_size}_v4tcn{artifact_suffix}.pth",
        )
        torch.save(
            {
                "temporal_type": temporal_type,
...
def run_for_window_size(
    ...
    *,
    ...
    with mlflow.start_run(
        run_name=f"rul_hybrid_v4_tcn_{run_tag}_{cli_feature_status}_ws{cfg.window_size}"
    ):
        mlflow.log_params(
            {
        ...
        for temporal_type in temporal_types:
            ...
            study = optuna.create_study(
                direction="minimize",
                study_name=(
                    f"rul_hybrid_v4_tcn_{run_tag}_{cli_feature_status}_"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Hybrid V4 TCN temporal RUL models.")
    parser.add_argument(
        "--fast",
        action="store_true",
        help=(
    ...
    print("\n" + "=" * 78)
    print(color_text(
        f"  TRAINING COMPLETE (HYBRID_V4_TCN) — {mode_label}",
        "green",
        bold=True,
    ))
    print("=" * 78)
