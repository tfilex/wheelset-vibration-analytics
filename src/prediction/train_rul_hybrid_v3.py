"""
train_rul_hybrid_v3.py — CNN+temporal pipeline для прогнозирования RUL.
...
Запуск (полный):
    uv run python src/prediction/train_rul_hybrid_v3.py --profile full

Запуск (быстрый sanity-check перед ночным прогоном):
    uv run python src/prediction/train_rul_hybrid_v3.py --profile fast

Запуск (рекомендуемый дневной режим):
    uv run python src/prediction/train_rul_hybrid_v3.py --profile balanced

Запуск с именем MLflow-эксперимента:
    uv run python src/prediction/train_rul_hybrid_v3.py --profile balanced --experiment-name "my_experiment"
...
MLFLOW_EXPERIMENT = "XJTU_SY_RUL_HybridTemporal_v3"
...
PREDS3_MODELS_DIR = os.path.join(MODELS_DIR, "rul_hybrid_v3_finetune")
...
FIGURES_BASE_DIR = "reports/figures/summary/rul_hybrid_v3_finetune"
...
def fit_and_save(
    ...
    *,
    ...
    if final_batch_size < hpo_batch_size:
        print(
            color_text(
                "[INFO] Final fit batch size reduced for unfrozen CNN: "
    ...
    train_loader = build_loader(
        train_ds, batch_size=final_batch_size, shuffle=True, device=device)
    val_loader = build_loader(
        val_ds, batch_size=final_batch_size, shuffle=False, device=device)
    test_loader = build_loader(
        test_ds, batch_size=final_batch_size, shuffle=False, device=device)
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for epoch in range(epochs):
            ...
            if val_mse < best_val_mse:
                ...
                epochs_no_improve += 1
                improved = False
            phase = "finetune" if encoder_unfrozen else "warmup"
            ...
            print(
                color_text(
                    ...
                    f"head_lr={lr_by_group.get('head', 0.0):.3g} "
                    f"encoder_lr={lr_by_group.get('encoder', 0.0):.3g} "
        ...
        ckpt_path = os.path.join(
            PREDS3_MODELS_DIR,
            f"best_rul_{temporal_type}_ws{window_size}_v3{suffix}.pth",
        )
        torch.save(
            {
                "temporal_type": temporal_type,
        ...
        if figures_dir is not None:
            ...
            rul_smooth_path = os.path.join(
                figures_dir,
                f"{temporal_type}_ws{window_size}_rul_prediction_smoothed{suffix}.png",
            )
            plot_rul_prediction(
                test_labels,
                test_preds_smoothed,
            ...
            res_smooth_path = os.path.join(
                figures_dir,
                f"{temporal_type}_ws{window_size}_residuals_smoothed{suffix}.png",
            )
            plot_residuals(
                test_labels,
                test_preds_smoothed,
...
def run_for_window_size(
    ...
    *,
    ...
    with mlflow.start_run(
        run_name=f"rul_hybrid_v3_{run_tag}_{cli_feature_status}_ws{cfg.window_size}"
    ):
        mlflow.log_params(
            {
        ...
        for temporal_type in temporal_types:
            ...
            study = optuna.create_study(
                direction="minimize",
                study_name=(
                    f"rul_hybrid_v3_{run_tag}_{cli_feature_status}_"
...
def main() -> None:
    ...
    print("\n" + "=" * 78)
    print(color_text(
        f"  TRAINING COMPLETE (RUL_HYBRID_V3) — {mode_label}",
        "green",
        bold=True,
    ))
    print("=" * 78)
